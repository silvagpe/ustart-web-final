...
export class OrcamentoComponent implements OnInit {
  ...
  public novo(): void {
    const url = `${AppRoutes.Orcamento.Cadastro()}/novo`;
  ...
  private pesquisar(pesquisa: string): void {
    this.loading = true;
    this.orcamentoService.get(pesquisa).subscribe(
      (result) => {
  ...
  public editar(orcamento: Orcamento): void {
    var url = `${AppRoutes.Orcamento.Cadastro()}/${orcamento.id}`;
    this.router.navigateByUrl(url);
  }

  public excluir(orcamento: Orcamento): void {
    if (confirm(`Deseja excluir o registro ${orcamento.id}?`)) {
